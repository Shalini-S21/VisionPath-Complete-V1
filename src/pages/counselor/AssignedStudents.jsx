  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-900 dark:text-white">Assigned Students Roster</h1>
          <p className="text-sm text-slate-500 mt-0.5">{students.length} students currently assigned under your counselor roster</p>
        </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 overflow-hidden">
        {loading ? (
          ...
          <div className="py-12 text-center text-slate-400 text-sm">No assigned students found.</div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
                  ...
                  <th className="text-left px-5 py-3 text-xs font-semibold text-slate-500">Institution / Degree</th>
                  <th className="text-left px-5 py-3 text-xs font-semibold text-slate-500">Academic Year</th>
                  ...
                  <tr key={student.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-xl bg-emerald-100 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-300 font-bold flex items-center justify-center text-xs">
                          {(student.name || student.username || 'S')[0].toUpperCase()}
                        </div>
                        <div>
                          <p className="text-xs font-bold text-slate-900 dark:text-white">{student.name || student.username}</p>
                    ...
                    <td className="px-5 py-3.5">
                      <span className="text-xs text-slate-500">{student.academicYear || '4th Year'}</span>
                    </td>
                    <td className="px-5 py-3.5">
                    ...
                    <td className="px-5 py-3.5 text-right">
                      <Link to={`/counselor/student/${student.id}`}>
                        <button className="inline-flex items-center gap-1 text-[11px] font-semibold text-emerald-600 hover:text-emerald-700 hover:underline">
